...
app = FastAPI(title="Модуль Методология")
...
def chats(db: Session = Depends(get_db), uid: int = Depends(_uid)):
    ...
    saved_text = ""
    if saved_last:
        if saved_last.msg_type == 'voice': saved_text = '🎤 Голосовое'
        elif saved_last.msg_type == 'file': saved_text = f'📎 {saved_last.file_name or "Файл"}'
        elif saved_last.msg_type == 'image': saved_text = '🖼️ Изображение'
        else: saved_text = dec(saved_last.enc_text)[:60]
...
def shared_content(other: int, kind: str = "media", db: Session = Depends(get_db), uid: int = Depends(_uid)):
    """Return shared media/files/voice between current user and `other`.
    kind: 'media' (image), 'files' (file), 'voice' (voice)."""
        ...
        while True:
            ...
            try:
                ...
                if gid:
                    ...
                    rid = int(rid) if rid else uid          # no recipient → Saved Messages (self chat)
                    if not db.query(User).filter(User.id==rid).first(): continue
                    if rid == uid:
                        # Saved Messages: delivered+read instantly, only echoed to self
                        m = Message(recipient_id=uid, is_read=True, **kw)
                        db.add(m); db.commit(); db.refresh(m)
                        out = {**_mdict(m), "event":"message", "temp_id":tmp}
                        targets = [uid]
                    else:
                ...
                db.close()
            # send AFTER releasing the DB session so delivery is never blocked on db
            for t in targets:
